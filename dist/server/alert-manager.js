...
(function (AlertType) {
    ...
    AlertType["CONSECUTIVE_BLOCKS_MISSED"] = "consecutive_blocks_missed";
...
class AlertManager extends events_1.EventEmitter {
    ...
    checkMetrics() {
        ...
        if (this.metrics.signStatus && this.metrics.signStatus.length > 0) {
            ...
            if (consecutiveMissed >= this.thresholds.consecutiveBlocksMissed) {
                if (!this.isMissingBlocks) {
                    ...
                    this.createAlert(AlertType.CONSECUTIVE_BLOCKS_MISSED, `⚠️ ALERT: ${consecutiveMissed} blocks missed`, 'warning');
                }
                else if (consecutiveMissed > this.lastAlertedConsecutiveMissed) {
                    // The number of missed blocks has increased
                    this.lastAlertedConsecutiveMissed = consecutiveMissed;
                    this.createAlert(AlertType.CONSECUTIVE_BLOCKS_MISSED, `🚨 ALERT: ${consecutiveMissed} blocks missed in increase`, 'critical');
                }
            }
        ...
        if (this.metrics.heartbeatStatus && this.metrics.heartbeatStatus.length > 0) {
            ...
            if (consecutiveHeartbeatsMissed >= this.thresholds.consecutiveHeartbeatsMissed) {
                if (!this.isMissingHeartbeats) {
                    ...
                    this.createAlert(AlertType.CONSECUTIVE_HEARTBEATS_MISSED, `⚠️ ALERT: ${consecutiveHeartbeatsMissed} heartbeats missed`, 'warning');
                }
                    ...
                    this.lastAlertedConsecutiveHeartbeatsMissed = consecutiveHeartbeatsMissed;
                    this.createAlert(AlertType.CONSECUTIVE_HEARTBEATS_MISSED, `🚨 ALERT: ${consecutiveHeartbeatsMissed} heartbeats missed in increase`, 'critical');
                }
            }
        ...
        if (signRate < this.thresholds.signRateThreshold) {
            if (!this.isLowSignRate) {
                ...
                this.createAlert(AlertType.SIGN_RATE_LOW, `⚠️ ALERT: Low signature rate (${signRate.toFixed(2)}%)`, 'warning');
            }
            else if (signRate < this.lastAlertedSignRate) {
                // The rate has decreased
                this.lastAlertedSignRate = signRate;
                this.createAlert(AlertType.SIGN_RATE_LOW, `🚨 ALERT: Signature rate in decrease (${signRate.toFixed(2)}%)`, 'critical');
            }
        }
        ...
        if (heartbeatRate < this.thresholds.heartbeatRateThreshold) {
            if (!this.isLowHeartbeatRate) {
                ...
                // The rate has decreased
                this.lastAlertedHeartbeatRate = heartbeatRate;
                this.createAlert(AlertType.HEARTBEAT_RATE_LOW, `🚨 ALERT: Heartbeat rate in decrease (${heartbeatRate.toFixed(2)}%)`, 'critical');
            }
        }
        ...
        Object.entries(this.metrics.evmVotes).forEach(([chain, chainData]) => {
            ...
            if (consecutiveMissed >= this.thresholds.consecutiveEvmVotesMissed) {
                if (!this.evmConsecutiveMissedByChain[chain]) {
                    ...
                    this.createAlert(AlertType.EVM_VOTE_MISSED, `⚠️ ALERT: ${consecutiveMissed} consecutive missed EVM votes on chain ${chain}`, 'warning', chain);
                }
                    ...
                    this.createAlert(AlertType.EVM_VOTE_MISSED, `🚨 ALERT: ${consecutiveMissed} consecutive missed EVM votes in increase on chain ${chain}`, 'critical', chain);
                }
            }
        ...
        Object.entries(this.metrics.ampdVotes).forEach(([chain, chainData]) => {
            ...
            if (consecutiveMissed >= this.thresholds.consecutiveAmpdVotesMissed) {
                if (!this.ampdVotesConsecutiveMissedByChain[chain]) {
                    ...
                    this.createAlert(AlertType.AMPD_VOTE_MISSED, `⚠️ ALERT: ${consecutiveMissed} consecutive missed AMPD votes on chain ${chain}`, 'warning', chain);
                }
                else if (consecutiveMissed > this.ampdVotesConsecutiveMissedByChain[chain]) {
                    this.ampdVotesConsecutiveMissedByChain[chain] = consecutiveMissed;
                    console.log(`Chain ${chain}: Increased to ${consecutiveMissed} missed votes, sending critical alert`);
                    this.createAlert(AlertType.AMPD_VOTE_MISSED, `🚨 ALERT: ${consecutiveMissed} consecutive missed AMPD votes in increase on chain ${chain}`, 'critical', chain);
                }
            }
        ...
        Object.entries(this.metrics.ampdSignings).forEach(([chain, chainData]) => {
            ...
            if (consecutiveMissed >= this.thresholds.consecutiveAmpdSigningsMissed) {
                if (!this.ampdSigningsConsecutiveMissedByChain[chain]) {
                    ...
                    this.createAlert(AlertType.AMPD_SIGNING_MISSED, `⚠️ ALERT: ${consecutiveMissed} consecutive missed AMPD signings on chain ${chain}`, 'warning', chain);
                }
                    ...
                    this.createAlert(AlertType.AMPD_SIGNING_MISSED, `🚨 ALERT: ${consecutiveMissed} consecutive missed AMPD signings in increase on chain ${chain}`, 'critical', chain);
                }
            }
            ...
            Object.keys(this.metrics.evmVotes).forEach(chain => {
                ...
                if (rate < this.thresholds.evmVoteRateThreshold) {
                    if (!this.evmVoteRateByChain[chain].isLow) {
                        ...
                        this.createAlert(AlertType.EVM_VOTE_RATE_LOW, `⚠️ ALERT: Low EVM vote rate (${rate.toFixed(2)}%) on chain ${chain}`, 'warning', chain);
                    }
                    else if (rate < this.evmVoteRateByChain[chain].lastRate) {
                        // The rate has decreased
                        this.evmVoteRateByChain[chain].lastRate = rate;
                        this.createAlert(AlertType.EVM_VOTE_RATE_LOW, `🚨 ALERT: EVM vote rate in decrease (${rate.toFixed(2)}%) on chain ${chain}`, 'critical', chain);
                    }
                }
            ...
            Object.keys(this.metrics.ampdVotes).forEach(chain => {
                ...
                if (rate < this.thresholds.ampdVoteRateThreshold) {
                    if (!this.ampdVoteRateByChain[chain].isLow) {
                        ...
                        this.createAlert(AlertType.AMPD_VOTE_RATE_LOW, `⚠️ ALERT: Low AMPD vote rate (${rate.toFixed(2)}%) on chain ${chain}`, 'warning', chain);
                    }
                    else if (rate < this.ampdVoteRateByChain[chain].lastRate) {
                        // The rate has decreased
                        this.ampdVoteRateByChain[chain].lastRate = rate;
                        this.createAlert(AlertType.AMPD_VOTE_RATE_LOW, `🚨 ALERT: AMPD vote rate in decrease (${rate.toFixed(2)}%) on chain ${chain}`, 'critical', chain);
                    }
                }
            ...
            Object.keys(this.metrics.ampdSignings).forEach(chain => {
                ...
                if (rate < this.thresholds.ampdSigningRateThreshold) {
                    if (!this.ampdSigningRateByChain[chain].isLow) {
                        ...
                        this.createAlert(AlertType.AMPD_SIGNING_RATE_LOW, `⚠️ ALERT: Low AMPD signing rate (${rate.toFixed(2)}%) on chain ${chain}`, 'warning', chain);
                    }
                    else if (rate < this.ampdSigningRateByChain[chain].lastRate) {
                        // The rate has decreased
                        this.ampdSigningRateByChain[chain].lastRate = rate;
                        this.createAlert(AlertType.AMPD_SIGNING_RATE_LOW, `🚨 ALERT: AMPD signing rate in decrease (${rate.toFixed(2)}%) on chain ${chain}`, 'critical', chain);
                    }
                }
     ...
     */
    async sendDiscordNotification(message, alert) {
        try {
            ...
            await axios_1.default.post(this.notificationConfig.discord.webhookUrl, payload);
     ...
     */
    async sendTelegramNotification(message, alert) {
        try {
